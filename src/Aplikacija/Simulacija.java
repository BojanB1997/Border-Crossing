...
//            }
    ...
    public void createListaVozila() {
        ...
        for (int i = 0; i < 35; i++) {
            listaLicnihVozila.add(new LicnoVozilo(mapa, brojac));
            brojac++;
        }

        for (int i = 0; i < 5; i++) {
            listaAutobsa.add(new Autobus(mapa, brojac));
            brojac++;
        }

        for (int i = 0; i < 10; i++) {
            listaKamiona.add(new Kamion(mapa, brojac));
            brojac++;
        }
    ...
    public Vozilo pretraziListu(Integer id) {
        ArrayList<Vozilo> kopijaListe = listaVozila;
        Optional<Vozilo> trazenoVozilo = kopijaListe.stream()
                .filter(vozilo -> vozilo.getIdd().equals(id))
                .findFirst();
        if (trazenoVozilo.isPresent()) {
            return trazenoVozilo.get();
        } else {
            return null;
        }
